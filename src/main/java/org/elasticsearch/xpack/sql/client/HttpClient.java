...
public class HttpClient
{
    ...
    public HttpClient(final ConnectionConfiguration cfg) {
    ...
    public SqlQueryResponse queryInit(final String query, final int fetchSize) throws SQLException {
        final SqlQueryRequest sqlRequest = new SqlQueryRequest(query, TimeValue.timeValueMillis(this.cfg.queryTimeout()), TimeValue.timeValueMillis(this.cfg.pageTimeout()), new RequestInfo(Mode.PLAIN, "cli"));
    ...
    private static <Request extends ToXContent> byte[] toXContent(final Request xContent) {
        try {
            ...
            try {
                final XContentBuilder xContentBuilder = new XContentBuilder(HttpClient.REQUEST_BODY_CONTENT_TYPE.xContent(), buffer);
                try {
